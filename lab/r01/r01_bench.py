...
SEQ_LEN = 256
PREFIX_K = 30          # R0's frozen semantic_rerank_k; R0.1 passes --prefix-k 10
WARMUP = 20
...
def run(model_dir: Path, catalog: Path, rep: int, prefix_k: int = PREFIX_K) -> dict:
    ...
    diag = {}
    for b in [x for x in BATCH_DIAGNOSTICS if x <= prefix_k] or [prefix_k]:
        samples = []
        for q in queries[:16]:
            t = time.perf_counter()
    ...
    return {
        "rep": rep, "model_dir": model_dir.name, "prefix_k": prefix_k,
        "cold_load_s": round(cold_s, 4),
        "artifact_load_s": round(load_s, 4),
        "topk_p95_ms": round(statistics.quantiles(gated, n=20)[18], 4),
        "topk_median_ms": round(statistics.median(gated), 4),
...
def main() -> int:
    ...
    ap.add_argument("--prefix-k", type=int, default=PREFIX_K)
    a = ap.parse_args()
    try:
        out = run(Path(a.model_dir), Path(a.catalog), a.rep, a.prefix_k)
